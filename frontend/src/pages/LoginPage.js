...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    if (!tcKimlik || !password) { toast.error('TC Kimlik No ve şifre girin'); return; }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen flex" data-testid="login-page">
      <div className="hidden lg:flex lg:w-1/2 bg-[#0F3935] relative items-center justify-center">
        <div className="relative z-10 text-center text-white px-12">
          <img src="/alarko-logo.png" alt="Alarko Enerji" className="h-16 w-auto object-contain mx-auto mb-6" />
          <h1 className="text-4xl font-bold font-[Poppins] mb-4">Alarko Enerji</h1>
          <p className="text-emerald-200/80 text-lg">Yenilenebilir enerji yatırımlarınızı güvenle yönetin.</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-slate-50">
        <div className="w-full max-w-md">
          ...
          <h2 className="text-2xl font-bold text-slate-900 font-[Poppins] text-center mb-2">Yatırımcı Girişi</h2>
          <p className="text-slate-500 text-center mb-8 text-sm">TC Kimlik No ve şifrenizle giriş yapın.</p>
            ...
            <CardContent className="p-6">
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div>
                  ...
                  <Input type="password" placeholder="Şifrenizi girin" value={password} onChange={e => setPassword(e.target.value)} className="h-12" data-testid="investor-password-input" />
                </div>
