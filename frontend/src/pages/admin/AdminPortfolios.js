...
import { Search, ChevronDown, ChevronRight, Plus, Trash2, Wallet, Briefcase, TrendingUp, User, DollarSign } from 'lucide-react';
...
export default function AdminPortfolios() {
  ...
  const handleAdd = async () => {
    if (!addForm.project_id || addForm.shares < 1) { toast.error('Proje ve hisse adedi seçin'); return; }
    try {
  ...
  const handleDelete = async () => {
    ...
    if (!shares || shares <= 0) { toast.error('Geçerli bir adet girin'); return; }
  ...
  const addTlTotal = (addForm.shares || 1) * SHARE_PRICE;
    ...
    <AdminLayout>
      <div data-testid="admin-portfolios">
        <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 font-[Poppins]">Hisse Yönetimi</h1>
            <p className="text-slate-500 text-sm">{filtered.length} yatırımcı</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            { icon: Briefcase, label: 'Toplam Yatırım', value: `${totalInvested.toLocaleString('tr-TR')} TL`, color: 'text-sky-600 bg-sky-50' },
        ...
        <div className="space-y-3">
                ...
                <div className="border-t bg-slate-50/50 p-4" data-testid={`investor-detail-${inv.user_id}`}>
                  ...
                  {inv.portfolios.length > 0 ? (
                    <div className="space-y-2">
                      ...
                      <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100 mt-2">
                        <div className="grid grid-cols-3 gap-4 text-center text-sm">
                          <div><p className="text-xs text-emerald-600">Toplam Yatırım</p><p className="font-bold text-emerald-800">{inv.total_invested.toLocaleString('tr-TR')} TL</p></div>
                          <div><p className="text-xs text-emerald-600">Toplam Hisse</p><p className="font-bold text-emerald-800">{inv.total_shares}</p></div>
                          <div><p className="text-xs text-emerald-600">Aylık Getiri</p><p className="font-bold text-emerald-800">+{inv.total_monthly_return.toLocaleString('tr-TR')} TL</p></div>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center py-6 text-slate-400">
                      ...
                      <p className="text-sm">Bu yatırımcının hissesi bulunmuyor.</p>
                    </div>
                  )}
                </div>
              )}
          ...
          {filtered.length === 0 && (
            <Card className="border-0 shadow-sm rounded-2xl">
              <CardContent className="p-12 text-center text-slate-400">Yatırımcı bulunamadı</CardContent>
            </Card>
          )}
        </div>
      </div>

      {/* Add Share Dialog - Enhanced with TL equivalent */}
          ...
          <div className="space-y-4 pt-2">
            <div>
              ...
              <Select value={addForm.project_id} onValueChange={v => setAddForm(p => ({ ...p, project_id: v }))}>
                <SelectTrigger className="mt-1" data-testid="add-share-project"><SelectValue placeholder="Proje seçin" /></SelectTrigger>
                <SelectContent>
            ...
            {/* TL Equivalent Display */}
            <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200" data-testid="add-share-tl-info">
              <div className="flex items-center gap-2 mb-2">
                <DollarSign className="w-4 h-4 text-emerald-600" />
                <p className="text-sm font-medium text-emerald-800">TL Karşılığı</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-emerald-600">Hisse Fiyatı</p>
                  <p className="font-bold text-emerald-800">{SHARE_PRICE.toLocaleString('tr-TR')} TL</p>
                </div>
                <div>
                  <p className="text-xs text-emerald-600">Toplam Tutar</p>
                  <p className="font-bold text-emerald-800 text-lg">{addTlTotal.toLocaleString('tr-TR')} TL</p>
                </div>
              </div>
              <p className="text-xs text-emerald-600 mt-2">{addForm.shares || 1} hisse x {SHARE_PRICE.toLocaleString('tr-TR')} TL = {addTlTotal.toLocaleString('tr-TR')} TL</p>
            </div>
      ...
      {/* Delete Share Dialog - Enhanced with TL equivalent */}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {deleteDialog && (
            <div className="space-y-4 pt-2">
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-1.5">Kaç hisse silinsin?</label>
                ...
                <div className="flex gap-2 mt-2">
                  {[1, Math.ceil((deleteDialog.shares || 1) / 2), deleteDialog.shares || 1].filter((v, i, a) => a.indexOf(v) === i && v > 0).map(n => (
                    <Button key={n} size="sm" variant="outline" className="text-xs rounded-lg" onClick={() => setDeleteShares(String(n))} data-testid={`delete-quick-${n}`}>
                      {n === (deleteDialog.shares || 1) ? 'Tümü' : `${n} hisse`}
                    </Button>
                  ))}
                </div>
              </div>

              {/* TL Equivalent for Delete */}
              {parseInt(deleteShares) > 0 && parseInt(deleteShares) <= (deleteDialog.shares || 1) && (
                <div className="p-3 rounded-xl bg-red-50 border border-red-200" data-testid="delete-tl-info">
                  <div className="flex items-center gap-2 mb-1">
                    <DollarSign className="w-4 h-4 text-red-600" />
                    <p className="text-sm font-medium text-red-800">Silinecek TL Karşılığı</p>
                  </div>
                  <p className="text-lg font-bold text-red-800">
                    {(parseInt(deleteShares) * SHARE_PRICE).toLocaleString('tr-TR')} TL
                  </p>
              ...
              <div className="flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => setDeleteDialog(null)} data-testid="delete-cancel-btn">İptal</Button>
