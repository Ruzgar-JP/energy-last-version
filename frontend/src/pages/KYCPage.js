...
export default function KYCPage() {
  ...
  const handleUpload = async () => {
    if (!frontFile || !backFile) { toast.error('Lütfen kimlik ön ve arka yüzünü yükleyin'); return; }
    ...
    try {
      ...
      toast.success('Kimlik belgeleri yüklendi. Admin incelemesi bekleniyor.');
      refreshUser();
  ...
  const statusMap = {
    approved: { icon: CheckCircle2, text: 'Onaylandı', color: 'text-emerald-600 bg-emerald-50', badge: 'bg-emerald-100 text-emerald-700' },
    submitted: { icon: Clock, text: 'İnceleniyor', color: 'text-amber-600 bg-amber-50', badge: 'bg-amber-100 text-amber-700' },
  ...
  return (
    <div className="min-h-screen bg-slate-50" data-testid="kyc-page">
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-8 pt-24 pb-12">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-slate-900 font-[Poppins] mb-2">Kimlik Doğrulama</h1>
        <p className="text-slate-500 mb-8">Yatırım yapabilmek için kimlik belgenizi doğrulayın.</p>
        ...
        <Card className="border-0 shadow-sm rounded-2xl mb-6">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-slate-500">Doğrulama Durumu</p>
        ...
        {(kycData?.kyc_status === 'pending' || kycData?.kyc_status === 'rejected' || !kycData?.kyc_status || user?.kyc_status === 'pending' || user?.kyc_status === 'rejected') && (
          <Card className="border-0 shadow-sm rounded-2xl">
            <CardContent className="p-6">
              <h3 className="font-semibold text-slate-900 font-[Poppins] mb-4">Kimlik Belgesi Yükle</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-slate-700 mb-2">Kimlik Ön Yüzü</label>
                  <div className="relative">
                    ...
                    <div className="w-full h-24 border-2 border-dashed rounded-xl flex items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors">
                      {frontFile ? (
                        ...
                        <span className="text-sm text-slate-400 flex items-center gap-2"><Upload className="w-4 h-4" /> Dosya seçin</span>
                      )}
                    </div>
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-slate-700 mb-2">Kimlik Arka Yüzü</label>
                  <div className="relative">
                ...
                <Button className="w-full h-12 bg-[#0F3935] hover:bg-[#0F3935]/90 text-white" onClick={handleUpload} disabled={loading} data-testid="kyc-submit-btn">
                  {loading ? 'Yükleniyor...' : 'Kimlik Belgelerini Gönder'}
                </Button>
              </div>
            </CardContent>
          </Card>
        ...
        {kycData?.kyc_document && kycData.kyc_status === 'approved' && (
          <Card className="border-0 shadow-sm rounded-2xl">
            <CardContent className="p-6 text-center">
              ...
              <h3 className="font-semibold text-slate-900 font-[Poppins] mb-2">Kimliğiniz Onaylandı</h3>
              <p className="text-slate-500 text-sm">Artık yatırım yapabilirsiniz. Projeleri inceleyerek başlayın.</p>
              <Link to="/projects"><Button className="mt-4 bg-emerald-500 hover:bg-emerald-600 text-white" data-testid="go-invest-btn">Projeleri İncele</Button></Link>
            </CardContent>
          </Card>
        )}
      </div>
